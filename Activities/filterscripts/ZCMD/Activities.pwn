	...
	if(InAct[playerid][ActIn])
	{
	    ...
	    if(ActInfo[Active] == TWarActive)
		{
			...
			if(ActInfo[GrovePlayers] >= 1 && !ActInfo[BallasPlayers])
			{
				...
				Message(-1, Green, "!$%s ניצחה בפעילות וכל הקבוצה קיבלה Grove הקבוצה", GetNum(ActInfo[Reward]));
			}
			if(ActInfo[BallasPlayers] >= 1 && !ActInfo[GrovePlayers])
			{
				...
				ActInfo[BallasPlayers] = 0;
				ActInfo[GrovePlayers] = 0;
				Message(-1, Purple, "!"green"$%s "purple"ניצחה בפעילות וכל הקבוצה קיבלה Ballas הקבוצה", GetNum(ActInfo[Reward]));
			}
		}
		if(ActInfo[Players] == 1 && ActInfo[Started])
		{
			...
			loop(i) if(InAct[i][ActIn])
			{
				...
				ResetPlayerWeapons(i);
				GivePlayerMoney(i, ActInfo[Reward]);
				Message(-1, Yellow, "!"green"$%s "yellow"ניצח בפעילות וקיבל "red"\"%s\" "yellow"השחקן", GetNum(ActInfo[Reward]), GetName(i));
			}
		}
	}
	return 1;
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
    if(InAct[playerid][ActIn] && ActInfo[Started] && ActInfo[Active] == SWarActive)
	{
		...
		ActInfo[Players]--;
		SendClientMessage(playerid, -1, ".יצאת מן הרכב ולכן הפסדת בפעילות");
		if(ActInfo[Players])
		{
		    KillTimer(ActInfo[Timer]);
			ActInfo[Players] = 0;
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DIALOG_STARTACT && response)
    {
        switch(listitem)
        {
            ...
            case 6: cmd_startflame(playerid, "");
            case 7: cmd_startchain(playerid, "");
        }
        return 1;
    }
    ...
    if(dialogid == DIALOG_REWARD && response)
    {
		...
		{
		    if(sscanf(inputtext, "i", ActInfo[Reward])) return cmd_startflame(playerid, "");
		...
		Message(-1, Orange, "/Join %d הפעילות החלה! בכדי להצטרף הקש", ActInfo[RandomNum]);
		Message(-1, Orange, "!"green"$%s "orange":פרס למנצח", GetNum(ActInfo[Reward]));
...
public StartACT()
{
    if(ActInfo[CD] == 15 && (ActInfo[Players] != 30 || ActInfo[Players] != 31)) SendClientMessageToAll(Pink, "/Join נשארו 15 שניות לתחילת הפעילות, בכדי להצטרף הקש");
    if(!ActInfo[CD])
    {
        KillTimer(ActInfo[Timer]);
...
public AutoAct()
{
	...
	SendClientMessageToAll(Red, "!מספר המקומות מוגבל לשלושים שחקנים בלבד, הרשמו מהר");
	SendClientMessageToAll(Orange, "--------------------------------------");
	ActInfo[Timer] = SetTimer("StartACT", 1000, 1);
	ActInfo[CD] = 30;
	return 1;
}
...
CMD:act(playerid, params[])
{
	...
	return ShowPlayerDialog(playerid, DIALOG_ACT, DIALOG_STYLE_MSGBOX, "מצב הפעיליות בשרת", String, "אישור", "");
}
...
{
    if(!IsPlayerAdmin(playerid)) return 0;
    if(!ActInfo[Active]) return SendClientMessage(playerid, Red, ".יש פעילות שפועלת, המתן לסיומה");
	ActInfo[Players] = 0;
	ActInfo[GrovePlayers] = 0;
	ActInfo[BallasPlayers] = 0;
	ActInfo[Active] = 0;
...
CMD:join(playerid, params[])
{
	...
	if(ActInfo[Started]) return SendClientMessage(playerid, -1, ".מצטערים, הפעילות כבר החלה");
	if(ActInfo[Players] >= 30) return SendClientMessage(playerid, Red, ".מצטערים, כל המקומות נתפסו");
	...
	if(ActInfo[Active] == TWarActive)
	{
		...
		SendClientMessage(playerid, -1, InAct[playerid][TWarPlayerID] == GroveTeam ? (""green"Grove :קבוצה") : (""purple"Ballas :קבוצה"));
	}
...
{
	if(!IsPlayerAdmin(playerid)) return 0;
	if(ActInfo[Active] != 0) return SendClientMessage(playerid, -1, ".יש פעילות שפועלת, המתן לסיומה");
...
{
    if(ActInfo[Active] != TWarActive) return SendClientMessage(playerid, -1, ".הפעילות אינה פעולת כעת");
    if(!ActInfo[Started]) return SendClientMessage(playerid, -1, ".הפעילות עוד לא החלה");
    String[0] = EOS;
...
//============================= [ Flame ] ====================================
CMD:startflame(playerid, params[])
{
	if(!IsPlayerAdmin(playerid)) return 0;
	if(ActInfo[Active] != 0) return SendClientMessage(playerid, -1, ".יש פעילות שפועלת, המתן לסיומה");
...
//============================= [ Flame ] ====================================
CMD:startchain(playerid, params[])
{
	if(!IsPlayerAdmin(playerid)) return 0;
	if(ActInfo[Active] != 0) return SendClientMessage(playerid, -1, ".יש פעילות שפועלת, המתן לסיומה");
