...
public OnPlayerDeath(playerid, killerid, reason)
{
    if(pM[playerid][InPizza])
	{
		...
		{
   			format(g_String, sizeof g_String, "[Pizza Mission] >> ! ðäøâ / éöà îï äøëá %s äùç÷ï", GetName(playerid));
		    SendClientMessageToAll(-1, g_String);
		}
	}
	return true;
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    if(oldstate == PLAYER_STATE_DRIVER)
	{
	    if(newstate == PLAYER_STATE_ONFOOT)
	    {
		    if(pM[playerid][InPizza])
			{
			    ...
			    pM[playerid][InPizzaVehicle] = false;
				return SendClientMessage(playerid, 0xFF0000FF, ".éù ìê 10 ùðéåú ìçæåø ìàåôðåò åìà úôñéã");
			}
		}
	}
	...
	if(newstate == PLAYER_STATE_DRIVER)
	{
	    ...
	    {
	        SendClientMessage(playerid, -1, ".éù ùç÷ï ùîáöò îùìåç áàåôðåò æä");
	        return RemovePlayerFromVehicle(playerid);
	    }
    	else if(!pM[playerid][InPizza])
    	{
    	    for(new i; i < 7; i++) if(vehicleid == PizzaVehicle[i])
		    	return ShowPlayerDialog(playerid, DIALOG_PIZZA, DIALOG_STYLE_MSGBOX, "Pizza Mission", "áîùéîä æå úöèøê ìäòáéø îùìåç ôéöä ìáéú ðáçø áæîï\nàí úñôé÷ ìäòáéø àú äôéöä áæîï úæëä áëñó\nàí ìà úñôé÷ úôñéã áîùéîä", "äúçì", "áéèåì");
		}
		else
		{
		    for(new i; i < 7; i++) if(vehicleid == PizzaVehicle[i])
...
{
	if(pM[playerid][InPizza])
	    return SendClientMessage(playerid, -1, ".àéðê éëåì ìáöò ô÷åãåú ëàùø àúä áîùéîú äôéöä");
